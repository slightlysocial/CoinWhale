...
class Xml {
    ...
    function generateTag($arr,$lvl = 0,$prev_lvl = 0) {
        ...
        foreach ($arr as $key => $value) {
            if (is_numeric($key)) {
                ...
                $this->XML .= $this->indent($lvl) . '<' . $this->strToXMLKeySafe($key,$lvl) . '>';

                if (is_array($value))   $this->XML .= $this->generateTag($value,$lvl + 1,$lvl);
                else                    $this->XML .= $this->strToXMLSafe((is_null($value) ? "NULL" : $value));

                // Remove attributes from closing key
                list($key) = explode(' ',$key);

                if (is_array($value)) $this->XML .= $this->indent($lvl);
                $this->XML .= '</' . $this->strToXMLKeySafe($key,$lvl) . '>' . $this->XMLLineBreak;
     ...
     * @access Private
     */
    function strToXMLKeySafe($str, $lvl = null) {
        if (is_numeric($str)) {
